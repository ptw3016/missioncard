<template>
  <section id="location" class="scroll-mt-24 py-20 px-4 relative overflow-hidden">
    ...
    <div class="container mx-auto px-2 md:p-4 relative z-10">
      <div>
        ...
        <div v-else class="bg-white bg-opacity-25 backdrop-blur-lg p-6 md:p-16 rounded-3xl shadow-2xl border border-white border-opacity-40 transform hover:rotate-x-1 transition-transform duration-700">
          ...
          <div class="text-center mb-14 relative">
            ...
            <p class="text-gray-600 max-w-2xl mx-auto text-lg">
              성령의 바람을 타고 <br class="sm:hidden">전국 각지로 "흩어지는 예배"  <br class="sm:hidden">참여교회를 확인하세요
            </p>
          </div>
          ...
          <div class="mt-1 flex flex-col md:flex-row justify-start">
            ...
            <!-- 왼쪽: 지도 + 네비게이션 바 컬럼 -->
            <div class="relative flex flex-col w-full md:w-1/2 mb-4 md:mb-0 md:mr-2 transition-all duration-300" data-delay="0.2">
              <!-- 도구 모음 - 개선된 컨트롤 패널 -->
              ...
              <div class="flex flex-col w-full h-full">
                 <div
                  class="h-96 md:h-[28rem] overflow-auto rounded-xl border border-white border-opacity-30 bg-white bg-opacity-20 backdrop-blur-md cursor-grab active:cursor-grabbing custom-scrollbar-transparent relative shadow-xl"
                  ref="imageContainer"
            ...
            <div
               ref="churchListRef"
               tabindex="0"
               class="flex flex-col flex-grow w-full md:w-1/2 md:ml-2 bg-white bg-opacity-20 backdrop-blur-sm rounded-xl shadow-lg border border-white border-opacity-30 p-3 md:p-6 transition-all duration-300 overflow-x-hidden overflow-y-auto h-auto md:h-[28rem]"
            ...
            >
              <div class="flex items-center mb-4">
                     ...
                     <h3 class="text-xl font-bold text-[#1e4e8f]">흩어지는 예배 <br class="sm:hidden">참여교회</h3>
              </div>
              <!-- Navigation Bar -->
              <div v-if="selectedButtonId" class="relative mt-4 mb-4 w-full top-0 z-10 bg-white/50 backdrop-blur-sm overflow-visible">
                  <!-- Mobile-only top '전체' & '담당별' Buttons -->
                  <div v-if="isMobile" class="flex space-x-2 px-2 mb-2">
                    <button
                      class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap flex items-center"
                      :class="selectedButtonId === null ? 'bg-gradient-to-r from-[#1e4e8f] to-[#2a9d8f] text-white shadow-md' : 'bg-blue-100 text-[#1e4e8f] hover:bg-opacity-80 border-[#1e4e8f] hover:text-white border'"
                      @click="goBack"
                    >전체</button>
                    <button
                      class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap flex items-center"
                      :class="selectedButtonId === 'diocese' ? 'bg-gradient-to-br from-yellow-400 to-yellow-200 text-black shadow-md' : 'bg-yellow-100 bg-opacity-100 text-gray-700 border border-yellow-300 hover:bg-opacity-60 bg-gradient-to-br from-yellow-400 to-yellow-200'"
                      @click="onMapButtonClick(buttonPositions.find(b => b.id === 'diocese'))"
                    ><i class="fas fa-church mr-2"></i>담당별</button>
                  </div>
                  <!-- Navigation Buttons Container -->
                  <div 
                    ref="mainNavRef" 
                    tabindex="0"
                    class="mb-4 px-2 py-2 overflow-x-auto custom-scrollbar-thin w-full"
                  >
                    <div class="flex space-x-2  min-w-max">
                      <!-- '전체' Button -->
                      <button
                        v-if="!isMobile"
                        class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:shadow-lg hover:scale-105 whitespace-nowrap flex items-center sticky left-0 z-20 border"
                        :class="selectedButtonId === null ? 'bg-gradient-to-r from-[#1e4e8f] to-[#2a9d8f] text-white shadow-md border-transparent' : 'bg-blue-100 text-[#1e4e8f] hover:bg-opacity-80 border-[#1e4e8f] hover:text-white'"
                        @click="goBack"
                      >전체</button>
                      <!-- Diocese Nav Button -->
                      <button
                        v-if="!isMobile"
                        class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:shadow-lg hover:scale-105 whitespace-nowrap flex items-center sticky left-[70px] z-20"
                        :class="selectedButtonId === 'diocese' ? 'bg-gradient-to-br from-yellow-400 to-yellow-200 text-black shadow-md' : 'bg-yellow-100 bg-opacity-100 text-gray-700 border border-yellow-300 hover:bg-opacity-60 bg-gradient-to-br from-yellow-400 to-yellow-200'"
                        @click="onMapButtonClick(buttonPositions.find(b => b.id === 'diocese'))"
                      ><i class="fas fa-church mr-2"></i>담당별</button>
                      <!-- Region Buttons -->
                      <button
                        ...
                        :key="btn.id"
                        class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:shadow-lg hover:scale-105 whitespace-nowrap flex items-center border border-transparent"
                        :class="selectedButtonId === btn.id ? 'bg-gradient-to-r from-[#1e4e8f] to-[#2a9d8f] text-white shadow-md' : 'bg-[#e0f2f1] bg-opacity-60 text-gray-700 hover:bg-opacity-80 hover:border-1 hover:border-[#1e4e8f]'"
                        @click="onMapButtonClick(btn)"
                        :data-id="btn.id"
                      >
                  ...
                  <transition name="dropdown">
                    <div 
                      v-if="selectedButtonId && regionButtons[selectedButtonId] && (!isMobile || showSubDropdown)" 
                      ref="dropdownRef"
                      class="w-full glass-panel-light pb-3 px-5 rounded-b-xl shadow-glass mt-1 z-20" 
                    >
                      <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 w-full">
                        <button
                          v-for="subBtn in sortedSubRegions"
                          :key="subBtn.id + '-sub'" 
                          :class="[
                            'sub-nav-button rounded-full shadow-lg transition-all duration-300 transform hover:shadow-xl hover:scale-110 hover:-translate-y-1 text-white w-full whitespace-nowrap px-2 md:px-1.5 lg:px-2 py-1 text-sm md:text-xs lg:text-sm',
                      ...
                      <div v-if="isMobile" class="flex justify-center mt-2">
                        <button @click="showSubDropdown = false" class="p-2 bg-white rounded-full shadow-lg hover:bg-opacity-80 transition-transform hover:scale-110">
                          <i class="fas fa-chevron-up text-[#1e4e8f]"></i>
                        </button>
                      </div>
                    </div>
                  </transition>
                  ...
                  <div 
                    v-if="isMobile 
                        && selectedButtonId 
                        && regionButtons[selectedButtonId] 
                        && !showSubDropdown" 
                    class="flex justify-center mt-2"
                  >
                    <button 
                      @click="showSubDropdown = true" 
                      class="p-2 bg-white rounded-full shadow-lg hover:bg-opacity-80 transition-transform hover:scale-110"
                    >
                      <i class="fas fa-chevron-down text-[#1e4e8f]"></i>
                    </button>
                  </div>
                </div>
              ...
              <div v-if="(selectedSubButtonId || selectedButtonId === 'frct') && !isDioceseListVisible" class="h-full flex flex-col">
                <!-- 헤더 (선택된 지역 이름 표시) -->
                <div class="flex items-center mb-4">
                  <div class="flex items-center rounded-full border border-[#217f73] pr-3">
                   <div class="w-10 h-10 rounded-full bg-green-100/10 flex items-center justify-center">
                      <i class="fas fa-seedling text-[#217f73]"></i>
                   </div>
                   <h3 class="text-lg font-bold text-[#217f73]">{{ getSelectedRegionName() }}</h3>
                  </div>
                </div>
                ...
                <!-- <div class="mb-6 bg-white bg-opacity-60 backdrop-blur-sm rounded-xl p-4 shadow-sm border-l-4 border-[#2a9d8f]">
                  <p class="text-gray-700 italic">
                    <i class=" <brfclass="sm:hidden">as fa-quote-left text-[#2a9d8f] mr-2 opacity-50"></i>
                    성령의 바람을 타고 <br class="sm:hidden">전국 각지로 "흩어지는 예배"  <br class="sm:hidden">참여여교회를 확인하세요
                  </p>
                </div> -->
                ...
                <div class="overflow-y-auto flex-grow custom-scrollbar-thin" ref="churchListScroll" tabindex="0">
                  ...
                  <div v-else-if="!isLoading && selectedButtonId === 'diocese' && Object.keys(dioceses).length > 0" class="space-y-6">
                    <div v-for="(churches, diocese) in dioceses" :key="diocese" class="bg-white bg-opacity-40 rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-200">
                      <h4 class="text-lg font-bold text-yellow-600 mb-2">{{ diocese }}</h4>
                      ...
                      <div class="flex items-start">
                        ...
                        <div class="flex-grow">
                          ...
                          <div v-if="expandedChurchId === church.id" class="mt-2 space-y-2">
                            ...
                            <!-- <p class="text-gray-600 text-sm mb-2"><strong>방문확인:</strong> {{ church.visit_confirmed === 'O' ? '확인됨' : '미확인' }} | <strong>섬지역:</strong> {{ church.is_island === '섬' ? '예' : '아니오' }}</p> -->
              ...
              <div v-else class="flex flex-col h-full overflow-auto p-2">
                  <!-- 네비게이션 메뉴 -->
                  <div class="flex-grow flex flex-col">
                     <!-- 도움말 추가 -->
                     <!-- <div class="mt-2 mb-8 bg-white bg-opacity-70 backdrop-blur-sm p-4 rounded-lg shadow-sm border-l-4 border-[#2a9d8f] max-w-2xl mx-auto">
                      <p class="text-gray-700 text-sm">
                        <i class="fas fa-info-circle mr-2 text-[#1e4e8f]"></i>
                        지역 버튼을 클릭하시면 해당 지역의 교회 정보를 확인하실 수 있습니다.
                      </p>
                    </div> -->
                    <div ref="mainNavRef" tabindex="0" class="grid h-auto md:h-full w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-2 md:gap-y-4 auto-rows-fr place-items-stretch md:flex-grow max-w-4xl">
                      
                      <!-- Diocese Nav Button -->
                      <button
                        class="w-full h-10 md:h-full whitespace-nowrap px-2 sm:px-3 md:px-4 py-2 rounded-2xl shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 sm:gap-1.5 md:gap-2"
                        :class="selectedButtonId === 'diocese'
                          ? 'bg-gradient-to-br from-yellow-500 to-yellow-300 text-white'
                          : 'bg-yellow-200/40 text-black border border-yellow-400 hover:bg-gradient-to-br from-yellow-400 to-yellow-200'"
                        @click="onMapButtonClick(buttonPositions.find(b => b.id === 'diocese'))"
                      >
                        <i class="fas fa-church"></i>담당별
                      </button>
                      ...
                      <button
                        v-for="btn in sortedButtonPositions.filter(b => b.id !== 'diocese')"
                        :key="btn.id"
                        class="w-full h-10 md:h-full whitespace-nowrap px-2 sm:px-3 md:px-4 py-2 rounded-2xl shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 text-xs sm:text-sm font-medium flex items-center justify-center gap-1 md:gap-1.5"
                        :class="btn.id === 'frct' ? 'border border-green-400 bg-green-400/0 text-green-400/100 hover:bg-gradient-to-br from-green-400 to-lime-300 text-green-400 hover:text-black/100' : 'bg-[#2a9d8f]/0 border border-[#1e4e8f] text-[#1e4e8f]/100 hover:bg-gradient-to-r from-[#1e4e8f] to-[#2a9d8f] hover:text-white'"
                        @click="onMapButtonClick(btn)"
                      >
  ...
  <!-- 모바일 모달 -->
  <div v-if="isMobile && showMobileModal && selectedButtonId" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    <!-- 배경 오버레이 -->
    <div class="absolute inset-0 bg-black bg-opacity-50" @click="showMobileModal = false"></div>
    
    <!-- 모달 컨텐츠 -->
    <div class="relative w-full max-w-md max-h-[80vh] bg-white rounded-xl shadow-2xl transform transition-all flex flex-col overflow-hidden">
      <!-- 모달 헤더 -->
      <div class="px-6 py-4 bg-gradient-to-r from-[#1e4e8f] to-[#2a9d8f] text-white sticky top-0 z-20">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <div class="w-10 h-10 rounded-full bg-white/0 flex items-center justify-center mr-0">
       ...
       <!-- Navigation Bar -->
       <div v-if="selectedButtonId" class="relative mt-4 mb-4 w-full bg-white/50 backdrop-blur-sm overflow-visible flex-none">
                  <!-- Mobile-only top '전체' & '담당별' Buttons -->
                  <div v-if="isMobile" class="flex space-x-2 px-2 mb-2">
                    <button
                      class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap flex items-center"
                    ...
                    <button
                      class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap flex items-center"
                      :class="selectedButtonId === 'diocese' ? 'bg-gradient-to-br from-yellow-400 to-yellow-200 text-black shadow-md' : 'bg-gradient-to-br from-yellow-100 to-yellow-100 bg-opacity-100 text-gray-700 border border-yellow-300 hover:bg-opacity-60 bg-gradient-to-br from-yellow-400 to-yellow-200'"
                      @click="onMapButtonClick(buttonPositions.find(b => b.id === 'diocese'))"
                    ><i class="fas fa-church mr-2"></i>담당별</button>
                  </div>
                  <!-- Navigation Buttons Container -->
                  ...
                  <transition name="dropdown">
                    <div 
                      ...
                      class="w-full glass-panel-light pb-3 px-5 rounded-b-xl shadow-glass mt-1 z-20" 
                    >
                      <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 w-full"> 
                        <button
                          v-for="subBtn in sortedSubRegions"
                          :key="subBtn.id + '-sub'" 
                          :class="[
                            'sub-nav-button rounded-full shadow-lg transition-all duration-300 transform hover:shadow-xl hover:scale-110 hover:-translate-y-1 px-2 py-1 text-sm text-white w-full',
                            selectedSubButtonId === subBtn.id
                              ? 'ring-4 ring-white z-20 bg-gradient-to-br from-[#2a9d8f] to-[#217f73]'
                              : 'bg-[#c2e9e5] text-gray-700 hover:bg-[#a5d8d3]'
                          ]"
      ...
      <!-- 모달 바디 -->
      <div class="p-2 overflow-y-auto flex-1 mx-2 mb-6 z-10">
        <!-- 로딩 중 표시 -->
